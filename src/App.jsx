...
import StatsPage from "./StatsPage"; // 상세 분석 페이지 추가
...
function App() {
  ...
  // 현재 페이지 상태(page)에 따라 렌더링할 컴포넌트를 결정하는 함수
  const renderPage = () => {
    if (page === "home") {
      return (
        <HomePage
    ...
    // 마이페이지 렌더링 시 setPage와 feedPosts를 넘겨주어 연동
    if (page === "mypage") {
      return <MyPage feedPosts={feedPosts} setPage={setPage} />;
    }

    // 상세 분석 통계 페이지 렌더링 (setPage를 통해 마이페이지로 복귀 가능)
    if (page === "stats") {
