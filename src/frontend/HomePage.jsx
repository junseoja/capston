...
function HomePage({
  ...
  // 상세 루틴 첨부 파일
  // 구조: { [루틴id]: [{ name, type, url, file }] }
  // - 한 루틴당 최대 3개까지 첨부 가능
  // - 기존 선택 파일은 유지한 채 추가 선택 가능
  ...
  // validateVideoDuration - 업로드된 영상 파일의 재생 시간 검증 (6초 이하)
  const validateVideoDuration = (file) => {
    return new Promise((resolve) => {
      ...
      video.onloadedmetadata = () => {
        const isValid = video.duration <= 6;
   ...
   */
  const handleFileChange = async (id, fileList) => {
    const selectedFiles = Array.from(fileList);
    const existingFiles = proofFiles[id] || [];

    if (selectedFiles.length === 0) return;

    const validatedNewFiles = [];

    for (const file of selectedFiles) {
      // 이미지 또는 영상만 허용
      if (!file.type.startsWith("image/") && !file.type.startsWith("video/")) {
        alert("이미지 또는 영상 파일만 추가할 수 있습니다.");
        continue;
      }

      // 영상은 6초 이내만 허용
      if (file.type.startsWith("video/")) {
        const isValidVideo = await validateVideoDuration(file);
        if (!isValidVideo) {
          alert(`"${file.name}" 영상은 6초 이내 파일만 업로드할 수 있습니다.`);
          continue;
        }
      }

      // 최대 3개까지만 추가
      if (existingFiles.length + validatedNewFiles.length >= 3) {
        alert("사진/영상은 최대 3개까지만 추가할 수 있습니다.");
        break;
      }
    ...
    if (validatedNewFiles.length === 0) return;

    setProofFiles((prev) => ({
      ...prev,
      [id]: [...(prev[id] || []), ...validatedNewFiles],
    }));
  };
  ...
  const handleDetailSubmit = async (id) => {
    ...
    const isCompleted = await onCompleteDetail(
      id,
      proofText,
      selectedFiles,
      uploadToFeed,
