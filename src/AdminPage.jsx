...
function AdminPage({ reports, onDeleteConfirm, notices, setNotices }) {
  ...
  const [selectedChallenge, setSelectedChallenge] = useState(null); // 상세 조회용
  const [isChallengeModalOpen, setIsChallengeModalOpen] = useState(false); // 추가/수정 모달
  const [editingChallenge, setEditingChallenge] = useState(null); // 수정 모드 확인용
  ...
  const handleSaveNotice = () => {
    if (!noticeTitle.trim() || !noticeContent.trim()) { alert("제목과 내용을 입력해주세요."); return; }
    if (editingNoticeId) {
      setNotices(notices.map(n => n.id === editingNoticeId ? { ...n, category: noticeCategory, title: noticeTitle, content: noticeContent } : n));
    } else {
      const newNotice = { id: Date.now(), category: noticeCategory, title: noticeTitle, content: noticeContent, date: new Date().toISOString().split('T')[0] };
  ...
  const handleDeleteNotice = (id) => { if (window.confirm("공지를 삭제하시겠습니까?")) setNotices(notices.filter(n => n.id !== id)); };
  ...
  const renderContent = () => {
    switch (currentMenu) {
            ...
            {selectedChallenge && (
              <div style={{ marginTop: "30px", background: "#fff", padding: "30px", borderRadius: "24px", border: "2px solid #4f46e5", boxShadow: "0 10px 30px rgba(79, 70, 229, 0.1)" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 2.5fr", gap: "25px" }}>
                  ...
                  <div style={{ background: "#f8f9fa", padding: "20px", borderRadius: "16px" }}>
                    <h4 style={{ fontSize: "14px", fontWeight: "800", marginBottom: "15px", color: "#4f46e5" }}>📸 실시간 인증 현황 (이미지/영상/내용)</h4>
                ...
                <tbody>
                  {filteredReports.map(report => (
                    <tr key={report.id} style={{ borderBottom: "1px solid #f3f4f6" }}>
                      <td style={{ padding: "16px 20px" }}>{report.user} / {report.postContent.title}</td>
                      <td>
                        <span style={{ padding: '4px 8px', borderRadius: '4px', fontSize: '11px', fontWeight: '800', backgroundColor: '#e6f7ff', color: '#1890ff' }}>
                          {getMostFrequentReason(report.reporters).match(/\[(.*?)\]/)?.[1] || "기타"}
                        </span>
                      </td>
                      <td style={{ textAlign: 'center', fontWeight: "800" }}>{report.reportCount}회</td>
                      <td style={{ textAlign: 'center' }}>{report.status === 'pending' ? '대기' : '완료'}</td>
                      <td style={{ padding: "16px 20px", textAlign: 'right' }}>
      ...
      case "notice":
        return (
          <div className="page-container" style={{ padding: "20px" }}>
            <div className="routine-header"><h2 className="routine-title">📢 공지사항 통합 관리</h2></div>
            <div style={{ display: 'flex', gap: '25px', marginTop: '25px', alignItems: 'flex-start' }}>
              <div style={{ flex: 1, background: "white", padding: "24px", borderRadius: "16px", border: editingNoticeId ? "2px solid #4f46e5" : "1px solid #eee" }}>
                <h3 style={{ fontSize: '15px', fontWeight: '900', marginBottom: '16px' }}>{editingNoticeId ? "📝 공지사항 수정" : "➕ 새 공지 등록"}</h3>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  <input type="text" value={noticeTitle} onChange={(e) => setNoticeTitle(e.target.value)} placeholder="공지 제목" style={{ padding: "10px", borderRadius: "8px", border: "1px solid #ddd" }} />
                  <textarea value={noticeContent} onChange={(e) => setNoticeContent(e.target.value)} placeholder="상세 내용" style={{ minHeight: "150px", padding: "14px", borderRadius: "8px", border: "1px solid #ddd" }} />
                  <button onClick={handleSaveNotice} className="routine-save-btn">{editingNoticeId ? "수정 완료" : "공지 게시"}</button>
                </div>
              </div>
              <div style={{ flex: 1, background: "white", padding: "24px", borderRadius: "16px", border: "1px solid #eee" }}>
                <h3 style={{ fontSize: '15px', fontWeight: '900', marginBottom: '16px' }}>📃 게시 내역</h3>
                <div style={{ maxHeight: '500px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  {notices.map(notice => (
                    <div key={notice.id} style={{ padding: '12px', border: '1px solid #f3f4f6', borderRadius: '10px' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '5px' }}>
                        <span style={{ fontSize: '11px', fontWeight: '800', color: '#4f46e5' }}>{notice.category}</span>
                        <div style={{ display: 'flex', gap: '8px' }}>
                           <button onClick={() => handleEditNotice(notice)} style={{ border: 'none', background: 'none', color: '#4f46e5', fontSize: '11px', cursor: 'pointer' }}>수정</button>
                           <button onClick={() => handleDeleteNotice(notice.id)} style={{ border: 'none', background: 'none', color: '#ff4d4f', fontSize: '11px', cursor: 'pointer' }}>삭제</button>
                        </div>
                      </div>
                      <div style={{ fontWeight: '800', fontSize: '13px' }}>{notice.title}</div>
                      <div style={{ fontSize: '11px', color: '#999' }}>{notice.date}</div>
                    </div>
                  ))}
                </div>
              </div>
      ...
      case "metrics":
        return (
          <div className="page-container" style={{ padding: "20px" }}>
            <div className="routine-header"><h2 className="routine-title">📊 서비스 활성 지표 분석</h2></div>
        ...
        return (
          <div className="page-container" style={{ padding: "20px" }}>
            <div className="routine-header">
              ...
              <p className="routine-subtitle">운영 현황 요약</p>
            </div>
